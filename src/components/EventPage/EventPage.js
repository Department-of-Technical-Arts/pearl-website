          ...
          <div className="card-container-competitions">
            {loaded && 
              events.map(
                (eachEvent, index) => {
                  if (loaded) {
                    return (<EventCard key={eachEvent.name} prize={eachEvent.prize} genre={eachEvent.genre} link={ eachEvent.details.startsWith("https") ? eachEvent.details : "https://" +  eachEvent.details} name={eachEvent.name} price={eachEvent.price} image={eachEvent.image_url} short={eachEvent.short}/>)
                  }
                }
              )
            }
